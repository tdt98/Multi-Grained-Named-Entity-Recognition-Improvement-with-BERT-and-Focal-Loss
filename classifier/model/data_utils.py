...
class CoNLLDataset(object):
    ...
    def __init__(self, filename, max_iter=None):
        ...
        self.roi_features = np.load(self.filename + 'roi_features.npy')
        #self.roi_elmo_features = np.load(self.filename + 'roi_elmo_features.npy')
        self.roi_bert_features = np.load(self.filename + 'roi_bert_features.npy')
        self.roi_lens = np.load(self.filename + 'roi_lens.npy')
        self.roi_labels = np.load(self.filename + 'roi_labels.npy')
        self.roi_char_ids = np.load(self.filename + 'roi_char_ids.npy', allow_pickle = True)
        self.roi_word_lengths = np.load(self.filename + 'roi_word_lengths.npy')
        self.sen_last_hidden = np.load(self.filename + 'sen_last_hidden.npy')
        ...
        self.right_context_word_lens = np.load(self.filename + 'right_context_word_len.npy')
    ...
    def get_data(self):
        ...
        for (features, bert_features, lens, labels, char_ids, word_lens, last_hidden, lf, ll, rf, rl) in self:
            total_features += [features]
            total_bert_features += [bert_features]
            total_lens += [lens]
            total_labels += [labels]
        ...
        return [total_features, total_bert_features, total_lens, total_labels, total_chars, total_word_lens, total_sen_last_hiddens, total_left_context_word_features, total_left_context_word_lens, total_right_context_word_features, total_right_context_word_lens]
...
def minibatches(dataset, minibatch_size):
    ...
    x_batch, bert_batch, len_batch, y_batch = [], [], [], []
    ...
    for (x, e, l, y, c, wl, lh, lc, ll, rc, rl) in zip(*dataset.data):
        if len(x_batch) == minibatch_size:
            ...
            yield x_batch, bert_batch, len_batch, y_batch, chars_batch, word_lens_batch, last_hidden_batch, lc_batch, ll_batch, rc_batch, rl_batch
            #x_batch, elmo_batch, len_batch, y_batch = [], [], [], []
            x_batch, bert_batch, len_batch, y_batch = [], [], [], []
            chars_batch, word_lens_batch, last_hidden_batch = [], [], []
            lc_batch, ll_batch, rc_batch, rl_batch = [],[], [], []
