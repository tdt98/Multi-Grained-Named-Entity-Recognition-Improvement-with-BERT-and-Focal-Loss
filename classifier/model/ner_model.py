...
class NERModel(BaseModel):
    ...
    def add_placeholders(self):
        ...
        #self.elmo_features = tf.placeholder(tf.float32,
         #       shape=[None, 3, self.config.anchor_types, 1024],
          #      name="elmo_features")
        # shape = (batch size, max_anchor_len, anchor_feature_len)
        self.bert_features = tf.placeholder(tf.float32, shape = [None, 3 , self.config.anchor_types, 1024], name = "bert_features")
    ...
    def get_feed_dict(self, features, bert_features, context_features,
        ...
        feed = {
            ...
            self.bert_features: np.array(bert_features),
        }
    ...
    def add_bert_op(self):
        with tf.variable_scope("bert"):
            ...
            batch_num = tf.shape(self.bert_features)[0]
            nsteps = tf.shape(self.bert_features)[2]
            norm_weights = tf.expand_dims(tf.expand_dims(tf.expand_dims(norm_weights, 0), 2), 3)
            norm_weights = tf.tile(norm_weights, [batch_num, 1, nsteps, 1024])

            # self.bert_embedding shape: [batch_size, 3, time_step, 1024]
            # bert_embedding shape: [batch_size, time_step, 1024]
            weighted_bert = tf.multiply(self.bert_features, norm_weights)
    ...
    def run_epoch(self, train, dev, epoch, dev_total_entity):
        ...
        for i, (features, bert_features, lens, labels, chars, word_lens, context_features, lc, ll, rc, rl) in enumerate(minibatches(train, batch_size)):

            fd = self.get_feed_dict(features, bert_features, context_features,
                    lc, ll, rc, rl,
    ...
    def run_evaluate(self, test, total_entity):
        ...
        for features, bert_features, lens, labels, chars, word_lens, context_features, lc, ll, rc, rl in minibatches(test, self.config.batch_size):
